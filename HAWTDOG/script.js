...
const handURL = 'Face.glb';
const steelURL = 'https://assets.codepen.io/911157/matcap_steel_256.jpg';
const purpleGlassURL = 'https://assets.codepen.io/911157/matcapPurpleGlass_256.png';
...
// GLOBAL DUMMY TO CONTROL TRANSFORMS
...
async function initInstancedHands() {
  ...
  iHand = new THREE.InstancedMesh(baseGeom, hand.material, handsCount);
  ...
  for (let i = 0; i < handsCount; i++) {
    ...
    dummy.scale.setScalar(5000.0);
    if (dreamball)
    {
      dummy.lookAt(dreamball.position);
      dummy.rotateX(THREE.MathUtils.degToRad(-90))
    }
    dummy.updateMatrix();
...
function initDreamball() {
  ...
  dogLoader.load('BigDog.glb',
                
    function (gltf) {
      dreamball = gltf.scene;
      scene.add(dreamball);
    },
